define('QuoteTransaction.Model', [
  ...
  return SCModel.extend({
    ...
    creditCardsPaymentsEnabled:
      ModelsInit.context.getSetting('FEATURE', 'CCTRACKING') === 'T',
    ...
    list: function (data) {
      ...
      const records = _.map(
        ...
        function (record) {
          const selected_currency = Utils.getCurrencyById(
            record.getValue('currency')
          )
    ...
    setCustomColumns: function (configName) {
      ...
      _.each(
        Configuration.get().transactionListColumns[configName],
        function (column) {
    ...
    setTransactionBodyCustomFields: function () {
      const self = this
      const customFieldsId = CustomFieldsUtils.getCustomFieldsIdToBeExposed(
        this.recordType
      )
      ...
      for (let i = 1; i <= this.record.getLineItemCount('promotions'); i++) {
        if (
          ...
          this.result.promocodes.push({
            ...
            // @property {String} internalid
            internalid: this.record.getLineItemValue(
              'promotions',
              'couponcode',
              i
            ),
            // @property {String} code
            code: this.record.getLineItemValue(
              'promotions',
              'couponcode_display',
              i
            ),
    ...
    getRecordSummary: function () {
      ...
      this.result.summary =
        // @class Transaction.Model.Get.Summary
        {
          ...
          estimatedshipping_formatted: Utils.formatCurrency(
            0,
            selected_currency_symbol
          ),
          ...
          taxonshipping_formatted: Utils.formatCurrency(
            0,
            selected_currency_symbol
          ),

          // @property {Number} discounttotal
          discounttotal: Utils.toCurrency(
          ...
          taxondiscount_formatted: Utils.formatCurrency(
            0,
            selected_currency_symbol
          ),

          // @property {Number} discountrate
          discountrate: Utils.toCurrency(
          ...
          discountedsubtotal_formatted: Utils.formatCurrency(
            0,
            selected_currency_symbol
          ),
    ...
    getLines: function () {
      ...
      for (let i = 1; i <= this.record.getLineItemCount('item'); i++) {
        if (
          ...
          this.result.lines[line_id] = this.result.lines[line_id]
            ...
            : {}
          this.result.lines[line_id].discount = this.result.lines[line_id]
            .discount
          ...
          this.result.lines[line_id].discount_name =
            this.record.getLineItemValue('item', 'item_display', i)
        } else {
          const rate = Utils.toCurrency(
            this.record.getLineItemValue('item', 'rate', i)
          )
          ...
          amount = Utils.toCurrency(
            this.record.getLineItemValue('item', 'amount', i)
          )

          const tax_amount =
            Utils.toCurrency(
              this.record.getLineItemValue('item', 'tax1amt', i)
            ) || 0
          ...
          this.result.lines[line_id] = {
            ...
            quantity: parseInt(
              this.record.getLineItemValue('item', 'quantity', i),
              10
            ),
            ...
            tax_code: this.record.getLineItemValue(
              'item',
              'taxcode_display',
              i
            ),
            ...
            shipmethod:
              this.record.getLineItemValue('item', 'shipmethod', i) || null,
            // @property {Number} index
            index: i,
            // @property {Boolean} isfulfillable
            free_gift: !!this.record.getLineItemValue(
              'item',
              'freegiftpromotion',
              i
            )
      ...
      _.each(this.result.lines, function (line) {
        line.rate_formatted = Utils.formatCurrency(
          line.rate,
          selected_currency_symbol
        )
        line.amount_formatted = Utils.formatCurrency(
          line.amount,
          selected_currency_symbol
        )
        ...
        line.total_formatted = Utils.formatCurrency(
          line.total,
          selected_currency_symbol
        )
    ...
    transactionModelGetLineOptionBuilder: function (
      internalId,
      label,
      value,
      mandatory
    ...
    loadItemsWithStoreItem: function (items_to_preload) {
      ...
      _.each(this.result.lines, line => {
        if (line.item) {
          ...
          if (
            line.type !== 'Discount' &&
            (!item || _.isUndefined(item.itemid))
          ) {
            items_to_query.push({
              id: line.item
            })
    ...
    getRecordShippingMethods: function () {
      ...
      for (let i = 1; i <= this.record.getLineItemCount('shipgroup'); i++) {
        self.addShippingMethod({
          ...
          shipcarrier: this.record.getLineItemValue(
            'shipgroup',
            'shippingcarrier',
            i
          )
    ...
    getRecordAddresses: function () {
      ...
      for (let i = 1; i <= this.record.getLineItemCount('iladdrbook'); i++) {
        ...
        this.result.listAddresseByIdTmp[
          ...
          country: this.record.getLineItemValue(
            'iladdrbook',
            'iladdrshipcountry',
            i
          ),
          state: this.record.getLineItemValue(
            'iladdrbook',
            'iladdrshipstate',
            i
          ),
          ...
          addr1: this.record.getLineItemValue(
            'iladdrbook',
            'iladdrshipaddr1',
            i
          ),
          addr2: this.record.getLineItemValue(
            'iladdrbook',
            'iladdrshipaddr2',
            i
          ),
          attention: this.record.getLineItemValue(
            'iladdrbook',
            'iladdrshipattention',
            i
          ),
          addressee: this.record.getLineItemValue(
            'iladdrbook',
            'iladdrshipaddressee',
            i
          ),
          phone: this.record.getLineItemValue(
            'iladdrbook',
            'iladdrshipphone',
            i
          )
    ...
    setAddress: function (prefix, address_id, other_address_name) {
      ...
      if (address_id) {
        if (!this.hasCurrentCustomerAddress(address_id)) {
          ...
          if (
            other_address_name &&
            this.data[other_address_name] === old_address_id
          ) {
